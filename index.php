...
<body>
    ...
    {
        // Replace space at line begin
        $text = preg_replace('/^[\s]+(.*)$/m', '$1', $text);

        // Replace headings
        $text = preg_replace('/^# (.*)$/m', '[header1]$1[/header1]', $text);
        $text = preg_replace('/^## (.*)$/m', '[header1green]$1[/header1green]', $text);
        ...
        // Replace list items
        $text = preg_replace('/^-\s+(.*)$/m', '[*] $1', $text);
        $text = preg_replace('/^[0-9]\.\s(.*)$/m', '[*] $1', $text);
    ...
    function convertCustomSyntaxToMarkdown($text)
    {
        ...
        $text = preg_replace('/\[header1green\](.*?)\[\/header1green\]/', '## $1', $text);

        // Replace bold text
        $text = preg_replace('/\[b\](.*?)\[\/b\]/', '**$1**', $text);
        $text = preg_replace('/\[i\](.*?)\[\/i\]/', '*$1*', $text);
        $text = preg_replace('/\[u\](.*?)\[\/u\]/', '__$1__', $text);
